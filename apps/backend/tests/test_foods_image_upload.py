...
def test_upload_valid_images(tmp_path) -> None:
    client = _auth_client()
    ...
    with override_settings(MEDIA_ROOT=tmp_path):
        response = client.post(
            ...
            {
                "image_large": SimpleUploadedFile(
                    "large.jpg", jpeg, content_type="image/jpeg"
                ),
                "image_small": SimpleUploadedFile(
                    "small.jpg", jpeg, content_type="image/jpeg"
                ),
...
def test_upload_oversize_rejected(tmp_path) -> None:
    ...
    with override_settings(MEDIA_ROOT=tmp_path):
        response = client.post(
            ...
            {
                "image_large": SimpleUploadedFile(
                    "large.jpg", oversize, content_type="image/jpeg"
                ),
                "image_small": SimpleUploadedFile(
                    "small.jpg", oversize, content_type="image/jpeg"
                ),
            },
            format="multipart",
        )
...
def test_upload_wrong_content_type_rejected(tmp_path) -> None:
    ...
    with override_settings(MEDIA_ROOT=tmp_path):
        response = client.post(
            ...
            {
                "image_large": SimpleUploadedFile(
                    "large.txt", b"hello", content_type="text/plain"
                ),
                "image_small": SimpleUploadedFile(
...
@pytest.mark.django_db
@pytest.mark.integration
def test_upload_missing_files_rejected(tmp_path) -> None:
    client = _auth_client()
    item = _create_food_item()
